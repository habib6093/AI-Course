...
def GBFS(start,destination):

       parent = {}
       cost = {}
       cost[start] = math.sqrt((romania_map_location[start][0]-romania_map_location[destination][0])**2 + (romania_map_location[start][1]-romania_map_location[destination][1])**2)
       visited={}
       visited[start]=1
       ...
       while temp !=None:
           ...
           for i in romania_map[temp]:
                   ...
                   if cost.get(i, 0) == 0 and visited.get(i,0) == 0:
       ...
       path.reverse()
       print("bfs path ",start," to ",destination," is: ",path)
